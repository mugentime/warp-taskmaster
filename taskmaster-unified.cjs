...
const WorkflowAuditor = require('./workflow-auditor-enhanced.cjs');
const TradeValidator = require('./trade-validator.cjs');
...
class TaskMasterUnified {
    constructor() {
        ...
        // Initialize trade validator for proper position sizing
        this.validator = new TradeValidator();
        console.log('🛡️ TRADE VALIDATOR ENABLED - Enforcing delta-neutral positions');
...
✅ DEPLOYMENT COMPLETE: ${successfulDeployments}/${positionsToAdd} successful`);
    ...
    async deployToOpportunity(opportunity, amount) {
        ...
        try {
            ...
            if (fundingRate < 0) {
                ...
                return await this.createShortPosition(symbol, amount);
            } else {
                // LONG futures for positive funding (we earn)
                return await this.createLongPosition(symbol, amount);
            }
    ...
    async createShortPosition(symbol, capitalAmount) {
        ...
        try {
            ...
            // BUY SPOT ASSET FIRST (delta-neutral requirement)
            ...
            // Calculate proper futures quantity with validator
            const quantityCalc = await this.validator.calculateFuturesQuantity(symbol, baseQty);
            if (!quantityCalc.success) {
                console.log(`   ❌ Failed to calculate futures quantity: ${quantityCalc.error}`);
                this.auditor.completeStep('POSITION_DEPLOYMENT', false);
                return false;
            }
            
            const futuresQuantity = quantityCalc.quantity;
            console.log(`   🔧 Futures quantity: ${futuresQuantity} (${(quantityCalc.hedgeRatio * 100).toFixed(1)}% hedge)`);
            
            // SHORT FUTURES WITH PROPER QUANTITY
            ...
            try {
                ...
                // Validate futures order with retries
                const validation = await this.validator.validateFuturesOrder(symbol, {
                    quantity: futuresQuantity,
                    orderId: futuresOrder.orderId
                });
                
                if (!validation.success) {
                    console.log(`   ❌ Futures validation failed: ${validation.error}`);
                    this.auditor.validate('POSITION_DEPLOYMENT', 'futures_hedge', false, validation);
                    this.auditor.completeStep('POSITION_DEPLOYMENT', false);
                    return false;
                }
                
                // Record successful hedge
                this.auditor.validate('POSITION_DEPLOYMENT', 'futures_hedge', true, validation);
                console.log(`   ✅ Futures: Shorted ${futuresQuantity} ${symbol.replace('USDT', '')} (${(validation.ratio * 100).toFixed(1)}% hedge)`);
                
                // Validate delta-neutral status
                const isDeltaNeutral = validation.ratio >= 0.90 && validation.ratio <= 1.10;
                this.auditor.validate('POSITION_DEPLOYMENT', 'delta_neutral_confirmed', isDeltaNeutral, validation);
                
                if (isDeltaNeutral) {
                    console.log(`   🎯 DELTA-NEUTRAL POSITION ESTABLISHED - Hedge Ratio: ${(validation.ratio * 100).toFixed(1)}%`);
                    this.auditor.completeStep('POSITION_DEPLOYMENT', true);
                    return true;
                } else {
                    console.log(`   ⚠️ WARNING: Position not properly hedged - Ratio: ${(validation.ratio * 100).toFixed(1)}%`);
                    this.auditor.completeStep('POSITION_DEPLOYMENT', false);
                    return false;
                }
                
            } catch (error) {
                console.log(`   ❌ Futures order failed: ${error.message}`);
                this.auditor.validate('POSITION_DEPLOYMENT', 'futures_hedge', false, { error: error.message });
                this.auditor.completeStep('POSITION_DEPLOYMENT', false);
                return false;
            }
