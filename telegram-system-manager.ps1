#!/usr/bin/env pwsh
# Complete Telegram System Manager
...
function Start-TelegramSystem {
    ...
    try {
        ...
        $reportsJob = Start-Job -Name "TelegramReports" -ScriptBlock {
            ...
            while ($true) {
                try {
                    $null = & node enhanced-telegram-reporter.js 2>&1
...
function Show-TelegramStatus {
    ...
    if (-not $telegramJobs -or $telegramJobs.Count -eq 0) {
        ...
        Write-Host ""
        Write-Host "💡 To start: .\telegram-system-manager.ps1 -Start" -ForegroundColor Cyan
        return
    }
    foreach ($job in $telegramJobs) {
...
# Main execution
Write-Host ""
Write-Host "📱 TELEGRAM SYSTEM MANAGER" -ForegroundColor Cyan
...
Write-Host "=========" -ForegroundColor Gray
Write-Host "  .\telegram-system-manager.ps1          # Show status" -ForegroundColor Gray
Write-Host "  .\telegram-system-manager.ps1 -Start   # Start both systems" -ForegroundColor Gray
Write-Host "  .\telegram-system-manager.ps1 -Stop    # Stop both systems" -ForegroundColor Gray
Write-Host "  .\telegram-system-manager.ps1 -Restart # Restart both systems" -ForegroundColor Gray
Write-Host ""
